...
public class UserProductPageCreator {
	...
	//페이지 번호화 한 페이지당 들어갈 게시물 수를 갖고 있는 객체
			...
			public String makeURI(Integer page) {
				UriComponents ucp = UriComponentsBuilder.newInstance()
									...
									.queryParam("keyword", ((ProductSearchVO)paging).getKeyword())
									.queryParam("condition", ((ProductSearchVO)paging).getCondition())
									.queryParam("category", ((ProductSearchVO)paging).getCategory())									
									.queryParam("priceSet", ((ProductSearchVO)paging).getPriceSet())									
									.queryParam("lowPrice", ((ProductSearchVO)paging).getLowPrice())
									.queryParam("highPrice", ((ProductSearchVO)paging).getHighPrice())
									.build();
