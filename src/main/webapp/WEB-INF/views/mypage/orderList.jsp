...
<div class="container">
	...
	<a href="<c:url value='/mypage/orderList?dateLength=1week&userNo=${orderSearch.userNo}'/>">
		<input type="button" value="1주일"  class="btn btn-outline-secondary btn-sm">
	</a>
	<a href="<c:url value='/mypage/orderList?dateLength=1month&userNo=${orderSearch.userNo}'/>">
		<input type="button" value="1개월" class="btn btn-outline-secondary btn-sm">
	</a>
	<a href="<c:url value='/mypage/orderList?dateLength=3month&userNo=${orderSearch.userNo}'/>">
		<input type="button" value="3개월" class="btn btn-outline-secondary btn-sm">
	</a>
	<a href="<c:url value='/mypage/orderList?dateLength=12month&userNo=${orderSearch.userNo}'/>">
		<input type="button" value="12개월" class="btn btn-outline-secondary btn-sm">
	</a>
	</div>
	<table class="table">
		<tr class="table-info">
		...
		<c:forEach var="orderDetail" items="${orderDetails}">
			...
			<tr class="table-light">
				...
				<c:if test="${orderDetail.orderList.orderStat==1}">
					<form action="<c:url value='/mypage/buyCancel'/>" method="post">
						...
						<input type="hidden" value="${orderDetail.orderInfo.userNo}" name="userNo">
					...
					<form action="<c:url value='/mypage/buyFinishExectute'/>" method="post">
						<input type="hidden" value="${orderDetail.orderList.proNo}" name="proNo">
						<input type="hidden" value="${orderDetail.orderList.orderNo}" name="orderNo">
	...
	<form action="<c:url value='/mypage/orderList'/>" method="get" class="row g-3 justify-content-end">
			<div class="col-auto">
				<input type="text" name="keyword" class="form-control" placeholder="제품명">
			</div>
